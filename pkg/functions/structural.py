...
def dihedrals(workdir: str):
    ...
    for group_number, handle in enumerate(handles):
        try:
            xvg_file = os.path.join(
                workdir,
                "analysis",
                "data_files",
                "Dihedrals",
                f"dihedrals_{handle}.xvg",
            )
            ...
            process = subprocess.Popen(
                [
                    ...
                    "angle",
                    "-n",
                    os.path.join(workdir, "angle.ndx"),
                    "-f",
                    os.path.join(workdir, "out", "traj.xtc"),
                    "-type",
                    "ryckaert-bellemans",
                    "-od",
                    xvg_file,
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(workdir, "analysis", "graphs", "Dihedrals", "dihedrals.png")
    )
...
def end_to_end(
    ...
    selected_atoms = mda_universe.select_atoms(
        f"resname {residue} and (type O or type C)"
    )
    ...
    for dt in mda_universe.trajectory:
        ...
        if len(upper_positions) != len(lower_positions):
            raise ValueError(
                f"Number of upper atoms ({len(upper_positions)}) doesn't match the number of lower atoms ({len(lower_positions)})"
            )
    ...
    plotting_backup.plot_line(
        f"{workdir}/analysis/graphs/End_to_End/End_to_end_{residue}.png",
        bin_centers,
        hist,
        title=title_text,
        xlabel="Distance (nm)",
    ...
    np.savetxt(
        f"{workdir}/analysis/data_files/End_to_End/End_to_end_{residue}.csv",
        np.column_stack([bin_centers, hist]),
        delimiter=",",
        header="Distance / nm, Frequency",
    )
...
def gyration(workdir: str):
    ...
    summary_file = os.path.join(
        workdir, "analysis", "graphs", "Gyration", "gyration_summary.txt"
    )
    ...
    try:
        ...
        for group_num, group_name in selected:
            ...
            try:
                t, rg_all, rg_x, rg_y, rg_z = np.loadtxt(
                    xvg_file, comments=["#", "@"], unpack=True
                )
...
def radial_density(
    mde_trajectory: Any, residue: str, atom: str, diameter: float
) -> NDArray:
    """
    ...
    pos = mde.distribution.time_average(
        partial(mde.distribution.radial_density, bins=bins),
        mde_trajectory.subset(atom_name=atom, residue_name=residue),
        segments=1000,
        skip=0.01,
    )
...
def radial_distances(
    mda_universe: Any, workdir: str, pore_diameter: float, exclusions: str = ""
) -> None:
    """
    ...
    for atom in unique_atoms:
        ...
        heatmap, xedges, yedges = np.histogram2d(
            data[:, 0], data[:, 1], bins=[x_bins, y_bins], density=True
        )
...
def rdf_inter(
    ...
    with tqdm(
        total=3 * len(set(residue_atoms.resids)),
        desc="Inter RDF Progress",
        unit="Resid",
    ) as pbar:
...
def rdf_intra(
    ...
    with tqdm(
        total=len(set(residue_atoms.resids)), desc="Intra RDF Progress", unit="Resid"
    ) as pbar:
        for resid in sorted(set(residue_atoms.resids)):
            a1_a2_rdf = InterRDF(
                g1=mdaUniverse.select_atoms(f"resid {resid} and name {atom1}"),
    ...
    return np.column_stack(
        [
            a1_a2_rdf.results.bins / 10,
            np.mean(rdf_array, axis=0) / np.max(np.mean(rdf_array, axis=0)),
        ]
    )
...
def z_align(vectors: NDArray, segments: int) -> NDArray:
    ...
    def z_angle_orientation(vectors, segments, skip=0.1):
        ...
        t, S = mde.correlation.shifted_correlation(
            partial(angles, z_vector=z_vector, bins=bins),
            vectors,
            segments=segments,
            skip=skip,
        )
...
def z_histogram(vectors: NDArray, segments: int, skip: float = 0.1) -> NDArray:
    ...
    t, S = mde.correlation.shifted_correlation(
        partial(z_comp, bins=bins), vectors, segments=segments, skip=skip
    )
    time = np.array([t_i for t_i in t for entry in x])
    angle = np.array([entry for t_i in t for entry in x])
    result = S.flatten()
