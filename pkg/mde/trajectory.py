...
from util.yaml import AnalysisLogger
from util.yaml import Params, Metadata, Results
...
class MDEvaluate:
    def __init__(
        ...
        self.PlottingMode = MDEvaluate._match_plotting_mode(PlottingMode)
        ...
        self._yaml_anal = AnalysisLogger(
            os.path.join(self.OutputDirectory, "Analysis_log.yaml")
        )
        self._yaml_plot = AnalysisLogger(
            os.path.join(self.OutputDirectory, "Plotting_log.yaml")
        )
        ...
        if Resolved:
            ...
            if not SkipCalculation:
                ...
                t, Results = mde.correlation.shifted_correlation(
                    ...
                    self.Coordinates,
                    selector=partial(MDEvaluate._MultiRadialSelector, rBins=rBins),
                ...
                self._yaml_anal.log(
                    ...
                    metadata=Metadata(
                        analysis_performed=datetime.now(),
                        plot_generated=datetime.now(),
    ...
    def vanHove(
        ...
        :param Buffer: Distance to buffer the radius beyond the diameter of system.
        """
        if not Translational and not Rotational:
            raise ValueError(
        ...
        if Translational:
            ...
            if not SkipCalculation:
                ...
                Header = "r / nm," + ",".join([f"{t / 1000:.0f} ns" for t in Time])
            ...
            if self.PlottingMode != "Off":
                ...
                SmoothedData = DataIn.iloc[:, 2:]
                SmoothedData = SmoothedData.rolling(window=15, center=True).mean()
        ...
        if Rotational:
            assert nBins > 0, "Please specify a quantity of angular bins to use."
            ...
            Filename = f"Etc/RotVanHove_{nBins}bins.csv"
            OutputFile = os.path.join(self.OutputDirectory, Filename)
            ...
            if not SkipCalculation:
                ...
                Header = "Angle / degrees," + ",".join(
                    [f"{t / 1000:.0f} ns" for t in Time]
                )
            ...
            if self.PlottingMode != "Off":
                ...
                SmoothedData = DataIn.iloc[:, 2:]
                SmoothedData = SmoothedData.rolling(window=30, center=True).mean()
    ...
    def zAxisAlignment(self, nBins: int = 1000):
        """
        Calculates residual vectors' alignment with Z axis.
        """
        SkipCalculation = False

        Filename = f"Etc/zAxis_Alignment_{nBins}bins.csv"
        OutputFile = os.path.join(self.OutputDirectory, Filename)
        ...
        if not SkipCalculation:
            ...
            def _Angles(start, end, zVector, aBins):
                angle = np.arccos((end * zVector).sum(axis=-1))
                angle = angle[(angle >= 0) & (angle <= np.pi)]
                histogram, _ = np.histogram(angle * 360 / (2 * np.pi), bins=aBins)
                return histogram / len(end)
            ...
            Header = "Angle / degrees," + ",".join([f"{t / 1000:.0f} ns" for t in Time])
    ...
    def zAxisRadialPos(self, nBins: int = 1000):
        """
        Calculates Z-axis radial positions.
        """
        SkipCalculation = False

        Filename = f"Etc/zAxis_Radial_Positions_{nBins}bins.csv"
        OutputFile = os.path.join(self.OutputDirectory, Filename)
        ...
        if not SkipCalculation:
            rBins = np.linspace(-1, 1, nBins)

            def z_comp(start, end, rBins):
                VectorsLengths = np.linalg.norm(end, axis=1)
                zComponent = end[:, 2] / VectorsLengths
                Histogram, _ = np.histogram(zComponent, rBins)
                return 1 / len(start) * Histogram

            Time, Result = mde.correlation.shifted_correlation(
                partial(z_comp, rBins=rBins),
                self.Vectors,
                segments=500,
                skip=0.05,
                points=8,
