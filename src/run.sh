...
run_python_script() {
    ...
    if [ ${#q_values[@]} -gt 0 ]; then
        for q in "${q_values[@]}"; do
            python run.py pore -dir "$WORKDIR" -tr 'out/traj.xtc' -tp 'run.tpr' -d "$DIAMETER" -s "$SEGMENTS" -a $ATOM1 $ATOM2 -res "$RESIDUE" -q "$q" \
                > >(tee "$LOGTMP") \
                2> >(tee -a "$LOGTMP" >&2)
        done
    else
            python run.py pore -dir "$WORKDIR" -tr 'out/traj.xtc' -tp 'run.tpr' -s "$SEGMENTS" -a $ATOM1 $ATOM2 -r "$RESIDUE" \
                > >(tee "$LOGTMP") \
                2> >(tee -a "$LOGTMP" >&2)
    fi
